...
def get_args():
    ...
    parser.add_argument(
        '-t', '--model_type',
        required=False, type=str, default='PVTv2',
...
def get_model():
    model = SegPVT(backbone_ckpt_path=CONFIG.checkpoint)
    model.to(DEVICE)
...
def get_data():
    ...
    train_dataset = SegGeneralDataset(
        CONFIG.data_path, split='train', point_sampler=None,
        augmenter=augmenter, **loader_params)
    val_dataset = SegGeneralDataset(
        CONFIG.data_path, split='val',  point_sampler=None,
        **loader_params)
