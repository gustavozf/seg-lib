"""
Source code imported from: https://github.com/DengPingFan/Polyp-PVT

LICENSE:
The source code is free for research and education use only.
Any comercial use should get formal permission first.
"""
...
from seg_lib.models.pvt_v2.pvt_v2_backbone import pvt_v2_b2
...
class SAM(nn.Module):
    def __init__(self, num_in=32, plane_mid=16, mids=4, normalize=False):
        ...
        self.conv_extend = (
            nn.Conv2d(self.num_s, num_in, kernel_size=1, bias=False)
        )
    ...
    def forward(self, x, edge):
        ...
        x_anchor = (
            self.priors(x_mask)[:, :, 1:-1, 1:-1].reshape(n, self.num_s, -1)
        )
        ...
        x_n_state = (
            torch.matmul(x_state_reshaped, x_proj_reshaped.permute(0, 2, 1))
        )
        if self.normalize:
...
class SegPVT(nn.Module):
    def __init__(self, backbone_ckpt_path: str, channel=32):
        super(SegPVT, self).__init__()

        self.backbone = pvt_v2_b2()  # [64, 128, 320, 512]
        save_model = torch.load(backbone_ckpt_path)
