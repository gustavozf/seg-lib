...
from seg_lib.models.sam.build_sam import sam_model_registry
...
class Transformer(nn.Module):
    ...
    def forward(self, x):      
        ...
        if self.training and self.rdrop > 0:
            ...
            rand_n = torch.rand((len(x), nlay), device=x.device)
            value_n = torch.linspace(0, self.rdrop, nlay, device=x.device)
            rdrop = (rand_n < value_n).float()
