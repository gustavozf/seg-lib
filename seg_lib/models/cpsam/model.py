...
from seg_lib.models.cpsam.vit_sam import Transformer
from seg_lib.models.cpsam.core import run_net, run_3D
...
class CellposeModel():
    ...
    def __init__(self, model_path: str = None, device: str = 'cpu'):
        ...
        Parameters:
            model_path (str or list of strings, optional): Full path to pretrained cellpose model(s), if None or False, no model loaded.
        ...
        if model_path is None:
            raise ValueError(
                ...
                "training from scratch is not implemented."
            )
        
        if not os.path.exists(model_path):
            raise FileNotFoundError(
                f"Pretrained model file {model_path} does not exist. "
                "Please provide a valid path to a pretrained model."
            )
    ...
    def __call__(self, img, **kwargs):
        """
        Call the model with an image and additional parameters.

        Parameters:
            img (numpy.ndarray): The input image to segment.
            **kwargs: Additional parameters for the eval method.

        Returns:
            tuple: A tuple containing masks, flows, and styles.
        """
        if isinstance(img, list) or img.squeeze().ndim == 5:
            # if img is a list, call eval on the list
            return self.call_many(img, **kwargs)

        return self.call_single(img, **kwargs)

    def call_single(
        self, img,
        batch_size=8, resample=True,
        ...
        anisotropy=None, flow3D_smooth=0,
        stitch_threshold=0.0, min_size=15,
        max_size_fraction=0.4, niter=None,
        augment=False, tile_overlap=0.1,
        bsize=256, compute_masks=True):
        # reshape image
        ...
        if nimg > 1 and do_normalization and (stitch_threshold or do_3D):
            ...
            if normalize_params["norm3D"] and nimg > 1 and do_normalization:
                # "normalize_params['norm3D'] is True but do_3D is False and 
                # stitch_threshold=0, so setting to False"
    ...
    def call_many(self, x, batch_size=8, resample=True, channel_axis=None,
             z_axis=None, normalize=True, invert=False, diameter=None,
        ...
        for i in tqdm(range(len(x))):
            ...
            maski, flowi, stylei = self.call_single(
                x[i], 
                batch_size=batch_size,
                channel_axis=channel_axis, 
                z_axis=z_axis,
    ...
    def _resize_cellprob(
            self,
            prob: np.ndarray, to_y_size: int,
            to_x_size: int, to_z_size: int = None
        ) -> np.ndarray:
        """
        ...
        if prob.ndim not in {2, 3}:
            raise ValueError(
                f'cellprobs have incorrect dimension after squeezing. '
                f'Should be 2 or 3, prob shape: {prob.shape}'
            )
